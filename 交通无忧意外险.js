...
describe('中国大地保险',function(){  
	...
	cy.visit(user.URL)
    cy.wait(2000)
    cy.contains('交通无忧意外险').click()
    cy.url().should('include', 'groupmessage')
    cy.wait(2000)
    cy.get('#wrapper > div.view.page-transition > main > div.travel-info > div.info-tab-body > div:nth-child(5) > div')
    .click()
    ...
    .type(phone+'@qq.com')
    cy.contains('￥20.00').should(($p) => {
                    expect($p).to.contain('￥20.00') 
                })
    cy.contains('确认信息').click()
    cy.get('#wrapper > div.view.page-transition > main > div.policyInfo > div:nth-child(9) > div > label > i').click()
